...
/**
 * Pure fetcher (no auth) — mirrors `funnel.ts`'s `_fetchFunnel`/`comisiones.ts`'s
 * `_fetchComisiones` shape (design.md ADR5, tasks.md Phase 18.2). PR3's
 * scorecard reuses this directly for the "interacciones" dimension (via
 * `rankingVendedores`), so that number can never drift from what this tab
 * shows. `obtenerReporteInteracciones` below is now a thin auth+dates
 * wrapper — no behavior change, regression-guarded by the pre-existing
 * `reportes-interacciones.test.ts` (PR1a Phase 4).
 *
 * NOT wrapped in `buildCachedReportFetcher` (ADR7: only fetchers being
 * restructured for pagination/exact-counts get the cache wrap in this
 * change; this extraction alone doesn't touch that concern).
 *
 * `periodo.fin` intentionally still resolves from `new Date().toISOString()`
 * (not `endISO`) — a documented pre-existing quirk (PR1a apply-progress,
 * deviation #2), preserved unchanged. Only the `.lte()` query bound (fixed
 * in PR1a) matters for the actual filtering.
 */
export async function _fetchInteracciones(
  supabase: SupabaseClient<any, "crm">,
  startISO: string,
  endISO: string,
): Promise<ReporteInteraccionesData> {
...
export async function obtenerReporteInteracciones(
  ...
  return safeAction(async () => {
    ...
    return _fetchInteracciones(supabase, startDate.toISOString(), endDate.toISOString());
  });
}
