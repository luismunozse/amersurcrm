...
const navigation: NavItem[] = [
  ...
  { name: "Clientes", href: "/dashboard/clientes", permisos: [PERMISOS.CLIENTES.VER_TODOS, PERMISOS.CLIENTES.VER_ASIGNADOS], icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  { name: "Proyectos", href: "/dashboard/proyectos", permisos: [PERMISOS.PROYECTOS.VER], icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
      </svg>
  )},
  { name: "Propiedades", href: "/dashboard/propiedades", permisos: [PERMISOS.LOTES.VER], icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  { name: "Documentos", href: "/dashboard/documentos", permisos: [PERMISOS.DOCUMENTOS.VER_TODOS, PERMISOS.DOCUMENTOS.VER_ASIGNADOS], icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  { name: "Mis Reportes", href: "/dashboard/vendedor/reportes", roles: ['ROL_VENDEDOR'], icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const adminNavigation: NavItem[] = [
  ...
  { name: "Marketing", href: "/dashboard/admin/marketing", roles: ['ROL_ADMIN'], icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z" />
      </svg>
  )},
  { name: "Reportes", href: "/dashboard/admin/reportes", permisos: [PERMISOS.REPORTES.GLOBALES], icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
      </svg>
  )},
  { name: "Configuración", href: "/dashboard/admin/configuracion", permisos: [PERMISOS.CONFIGURACION.SISTEMA], icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div className={cn("space-y-1.5", collapsed && "px-1")}>
      {Array.from({ length: count }).map((_, idx) => (
        <div
          ...
          className={cn(
            "h-11 rounded-xl bg-crm-sidebar-hover/40 animate-pulse",
            collapsed ? "lg:mx-1.5" : ""
          )}
        />
      ))}
