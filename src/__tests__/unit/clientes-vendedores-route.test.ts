...
describe("GET /api/clientes/vendedores", () => {
  it("returns the full vendedor+coordinador list for admin/gerente", async () => {
    ...
    expect(body.vendedores.map((v: any) => v.username).sort()).toEqual(["coord1", "vend1", "vend2"]);
  });

  it("scopes the list to only the caller's own team when the caller is a coordinador", async () => {
    vi.mocked(esAdminOCoordinador).mockResolvedValue(true);
    vi.mocked(esCoordinador).mockResolvedValue(true);
    const chain = createChainMock({ data: [rows()[0]], error: null }); // pre-filtered by .eq('coordinador_id', ...)
    mockServiceRoleClient.from = vi.fn(() => chain);

    const res = await GET();
    const body = await res.json();

    expect(chain.eq).toHaveBeenCalledWith("coordinador_id", "coord-1");
    expect(body.vendedores.map((v: any) => v.username)).toEqual(["vend1"]);
  });
