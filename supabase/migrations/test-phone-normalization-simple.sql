...
SELECT 
    ...
    telefono_whatsapp,
    CASE 
        WHEN telefono IS NOT NULL AND NOT telefono LIKE '+%' THEN 'SI'
        ELSE 'NO'
    END as telefono_necesita_normalizacion,
    CASE 
        WHEN telefono_whatsapp IS NOT NULL AND NOT telefono_whatsapp LIKE '+%' THEN 'SI'
        ELSE 'NO'
...
SELECT 'EJECUTANDO NORMALIZACION...' as info;
SELECT * FROM crm.normalize_phone_numbers();

-- Ver estadísticas después de la normalización
SELECT 'ESTADISTICAS DESPUES DE LA NORMALIZACION:' as info;
SELECT * FROM crm.get_phone_normalization_stats();
