-- Migración ultra simple para corregir valores NULL en DNI
-- Fecha: 2025-01-20
-- Descripción: Actualiza registros existentes con DNI NULL de forma ultra simple
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'crm' AND table_name = 'usuario_perfil') THEN
        ...
        -- Si hay registros con DNI NULL, actualizarlos con DNIs temporales
        IF v_count > 0 THEN
            ...
            -- Si hay duplicados, agregar sufijo a todos los duplicados
            UPDATE crm.usuario_perfil 
            SET dni = dni || '1'
            WHERE dni IN (
            ...
            RAISE NOTICE 'Duplicados corregidos con sufijo';
        END IF;
