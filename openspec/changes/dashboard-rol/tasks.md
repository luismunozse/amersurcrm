...
- [x] 11.1 `FunnelAgingBlock.tsx` + `.Skeleton.tsx` — funnel bar (reuses `getEstadoClienteLabel`, links each stage to `/dashboard/clientes?estado=X` mirroring `ReporteFunnel.tsx`'s existing convention) + aging counter (`getAgingLeads`, renders `count+` when `!isExact`) + a top-5 preview chip list, priority ①. **Deviation:** design's visual-hierarchy prose says the aging counter links to "the filtered pipeline", but `_PipelineBoard.tsx`'s urgency filter is client-side `useState`, not a URL query param — there is no pipeline URL that pre-filters to aging leads today (grep-verified: no `searchParams`/`urgencia=` handling in `page.tsx` or `_PipelineBoard.tsx`). Links to the unfiltered `/dashboard/pipeline` instead, same reasoning as PR1b's `LeadsSinContactar` link deviation (label matches actual destination).
- [x] 11.2 `InventarioLotesBlock.tsx` + `.Skeleton.tsx` — per-project bars (top 5 by total lotes, "+N restantes" link) + org totals summary in the header, links `/dashboard/proyectos` (priority ②). `vendido` renders `crm-success` here (management wants high `% vendido`) — a deliberately different color meaning than the operational `_LotesList.tsx`, which flags `vendido` red ("no longer sellable").
- [x] 11.3 `VentasVsMetaBlock.tsx` + `.Skeleton.tsx` — org `obtenerKPIs` sum (current + previous month in parallel) vs. goal + MoM delta (priority ③). Added two new pure helpers to `src/lib/dashboard/meta.ts` (TDD, RED→GREEN): `calcularDeltaMensual(actual, anterior)` (null when no prior-month sales to compare, avoids `+Infinity%`) and `calcularPeriodoAnterior(anio, mes)` (wraps December of the prior year from January) — both unit-tested in `dashboard-meta.test.ts` (13/13 passing incl. the 5 pre-existing `calcularProgresoMeta` cases). **No CTA link** (deviation from the pattern, but consistent with design's own silence on a link destination for this block): `/dashboard/admin/metas` is gated `soloAdmins()` (ROL_ADMIN only, verified in `page.tsx`), while this block also renders for gerente/coordinador — linking there would silently redirect them away. This is a pre-existing route-guard gap (the sidebar's own "Metas" nav entry already advertises this same broken destination to gerente/coordinador via a permission-only gate that the page's role-only `protegerRuta` doesn't honor) — flagged here, not fixed (out of scope for `dashboard-rol`).
- [x] 11.4 `MoraAlertasBlock.tsx` + `.Skeleton.tsx` — `obtenerResumenCobranza().monto_mora_total` (hardcoded `formatearMoneda(..., 'PEN')`, matching `_CobranzaList.tsx`'s own display exactly since `v_cobranza`'s aggregate has no per-row `moneda` column — required for the spec's "mora total matches the cobranza hub" scenario, not a shortcut) + `getAlertasSinGestionarCount`, links `/dashboard/cobranza?tab=alertas`, positive empty state when both are zero (priority ④).
- [x] 11.5 `src/app/dashboard/_components/CommandCenter.tsx`: composes the four blocks ①→④, each its own `<Suspense>`. Resolves `esGlobal` once via `getPerfilRol()` (design.md ADR-1) and passes it down to `FunnelAgingBlock`/`InventarioLotesBlock`/`MoraAlertasBlock` (`VentasVsMetaBlock` doesn't need it — `obtenerKPIs` already auto-scopes via `METAS.ASIGNAR`, confirmed granted to admin/gerente/coordinador in `20250326000008_permissions_matrix.sql`). `getPerfilRol` takes no arguments, so `React.cache` dedupes it for free if anything else in the request tree also calls it — no extra query cost from wiring it here, finally consuming the "forward-looking infra" built in PR1b.

### Phase 12: Legacy removal
- [x] 12.1 Deleted `src/app/dashboard/vendedor/page.tsx` and `src/app/dashboard/vendedor/loading.tsx` (grep-confirmed zero importers first — Next.js route files aren't imported directly anyway).
- [x] 12.2 Edited `src/app/auth/login/_LoginForm.tsx:329` — `router.replace("/dashboard/vendedor")` → `"/dashboard"`. The email/password login path (line 296) already targeted `/dashboard`; both paths now agree.
- [x] 12.3 Grep sweep: only remaining `/dashboard/vendedor` references are `/dashboard/vendedor/reportes` in `navigation.tsx`, `SidebarShadcn.tsx`, and `MetaDelMes.tsx` — all in-scope survivors, no dead links.

### Phase 11/12 extra — orphan fetcher cleanup (per apply-batch instructions, beyond tasks.md's original text)
- [x] Grep-reconfirmed post-PR2b (after `FunnelAgingBlock` wired `getCachedFunnelClientes` back in) that `getCachedDashboardStats`, `getCachedVentasMensuales`, and `getCachedClientesDashboardMetrics` still have **zero consumers** anywhere in `src/`. Deleted all three from `src/lib/cache.server.ts` (PR2a had left `getCachedDashboardStats` with an explanatory comment instead of deleting it, since Phase 8's ADR-2 fix was deliberately scoped narrowly to the coordinador-visibility fix, not a cleanup pass — this batch is that cleanup pass). Also deleted their now-solely-owned types: `ClienteDashboardMetrics` (private to `cache.server.ts`) and `DashboardStats` (`src/types/crm.ts`, confirmed zero other consumers). Left the `obtener_metricas_dashboard_clientes` Postgres RPC untouched (DB-side, no new schema per design's own constraint — dropping unused RPCs is a migration-level decision out of scope here).

### Phase 13: Gates
- [x] 13.1 `npx vitest run src/__tests__/unit/funnel-stats-coordinador.test.ts src/__tests__/unit/dashboard-aging.test.ts src/__tests__/unit/command-center-fetchers.test.ts src/__tests__/unit/dashboard-meta.test.ts src/__tests__/unit/dashboard-composition.test.ts src/__tests__/unit/dashboard-scope.test.ts src/__tests__/unit/seguimientos-hoy-fetcher.test.ts` — 63/63 passed. Additionally ran the entire `src/__tests__/unit` directory (53 files) as a safety net given the orphan cleanup touched a shared, widely-imported file (`cache.server.ts`) beyond what the narrow file list would catch — 697/697 passed.
- [x] 13.2 `npx tsc --noEmit` clean for the PR2b diff (also had to prune a stale generated `.next/types/app/dashboard/vendedor/page.ts` — a gitignored Next.js build-cache artifact from the deleted route, not a source file; it regenerates automatically on the next `next dev`/`next build`).
- [ ] 13.3 Full-suite gate (potentially-hanging in agent sandboxes — run with care): `npm test`. **Deliberately skipped per apply-batch instructions** (never run bare `npm test`/`next build` in this sandbox) — left for the orchestrator/user to run outside the sandbox as the final confirmation before merge.

## Status
All of PR1a, PR1b, PR2a, and PR2b are implemented and gated (Phases 1–13 checked off except the explicitly-deferred 13.3 full-suite run). `dashboard-rol` is ready for `sdd-verify`.
